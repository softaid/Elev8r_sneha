...
(function(){
sap.ui.define(["sap/esh/search/ui/SearchNavigationObject", "sap/esh/search/ui/SearchNavigationObjectForSinaNavTarget", "./SearchResultBaseFormatter", "./sinaNexTS/providers/abap_odata/UserEventLogger"], function (SearchNavigationObject, SearchNavigationObjectForSinaNavTarget, __SearchResultBaseFormatter, ___sinaNexTS_providers_abap_odata_UserEventLogger) {
    ...
    _createClass(SearchResultFormatter, [{
            ...
            switch (detailAttribute.metadata.type) {
              ...
              case sina.AttributeType.ImageUrl:
                ...
                if (detailAttribute.defaultNavigationTarget) {
                  formattedResultItem.imageNavigation = new SearchNavigationObjectForSinaNavTarget(detailAttribute.defaultNavigationTarget, this.model);
                }
              ...
              case sina.AttributeType.Group:
                {
                  var attributeGroupAsAttribute = this._formatAttributeGroup(detailAttribute, options,
                  /*index*/
                  z);
              ...
              case sina.AttributeType.Timestamp:
                {
                  var oItemAttribute = this._formatSingleAttribute(detailAttribute, options,
                  /*index*/
                  z);
      ...
      value: function _formatAttributeGroup(attributeGroup, options, index) {
        ...
        if (attributeGroup.attributes.length === 2 && typeof attributeGroup.attributes[1].attribute.value === "string" && attributeGroup.attributes[1].attribute.value.startsWith("sap-icon://")) {
          ...
          formattedAttr.displayOrder = attributeGroup.metadata.usage.Detail && attributeGroup.metadata.usage.Detail.displayOrder; // if (isLongtext) {
        ...
        var parentAttribute, childAttribute; // for (var attributeName in attributeGroup.attributes) {
      ...
      value: function _formatResultForDocuments(resultItem, additionalParameters) {
        ...
        if (additionalParameters.isDocumentConnector === true) {
          ...
          additionalParameters.titleUrl = "/sap/opu/odata/SAP/ESH_SEARCH_SRV" + sidClient + "/FileLoaderFiles(ConnectorId='" + connectorName + "',FileType='BinaryContent',SelectionParameters='" + keyFields + "')/$value"; // const suvlink = "/sap/opu/odata/SAP/ESH_SEARCH_SRV/FileLoaderFiles(ConnectorId='" + connectorName + "',FileType='SUVFile',SelectionParameters='PHIO_ID=" + resultItem.PHIO_ID.valueRaw + "')/$value?sap-client=" + client;
      ...
      value: function _formatResultForNotes( // eslint-disable-next-line @typescript-eslint/no-unused-vars
      resultItem, // eslint-disable-next-line @typescript-eslint/no-unused-vars
