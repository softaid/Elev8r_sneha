...
sap.ui.define(["./i18n", "sap/ui/model/Sorter", "sap/ui/core/library", "./hierarchydynamic/SearchHierarchyDynamicFacet", "./SearchFacetDialogHelperDynamicHierarchy", "./sinaNexTS/sina/SimpleCondition"], function (__i18n, Sorter, sap_ui_core_library, __SearchHierarchyDynamicFacet, ___SearchFacetDialogHelperDynamicHierarchy, ___sinaNexTS_sina_SimpleCondition) {
  ...
  var SearchFacetDialogHelper = /*#__PURE__*/function () {
    ...
    _createClass(SearchFacetDialogHelper, null, [{
      ...
      value: // ToDo class SearchAdvancedCondition
      ...
      value: function init(dialog) {
        ...
        SearchFacetDialogHelper.POS_FACET_LIST = 0; // every detailPage->subHeader->content[]

        SearchFacetDialogHelper.POS_TOOLBAR_SEARCHFIELD = 0;
        SearchFacetDialogHelper.POS_TOOLBAR_TOGGLEBUTTON = 1; // every detailPage->content[]
        // old layout (number, date facet)

        SearchFacetDialogHelper.POS_SETTING_CONTAINER = 0;
        SearchFacetDialogHelper.POS_ATTRIBUTE_LIST_CONTAINER = 1; // new layout (string, text facet)

        SearchFacetDialogHelper.POS_ICONTABBAR = 0; // tabBar->items[]

        SearchFacetDialogHelper.POS_TABBAR_LIST = 0;
        SearchFacetDialogHelper.POS_TABBAR_CONDITION = 1; // settingContainer->content[]

        SearchFacetDialogHelper.POS_SORTING_SELECT = 0;
        SearchFacetDialogHelper.POS_SHOWONTOP_CHECKBOX = 1; // advancedCondition->content[]
        ...
        SearchFacetDialogHelper.oIntegernumberFormat = sap.ui.core.format.NumberFormat.getIntegerInstance(); //format: 2015-07-14 00:00:00.0000000
      ...
      value: function updateDetailPage(oListItem, sFilterTerm, bInitialFilters) {
        ...
        if (sDataType === "string" || sDataType === "text") {
          ...
          oDetailPageAttributeList = oDetailPage.getContent()[SearchFacetDialogHelper.POS_ATTRIBUTE_LIST_CONTAINER] // ToDo 'any'
          .getContent()[0];
        ...
        oModel.chartQuery.filter.searchTerm = oSearchModel.getSearchBoxTerm(); // send the single call

        oModel.facetDialogSingleCall(properties).then(function () {
          var aItems = oModel.getProperty(oDetailPage.getBindingContext().getPath()).items; // initiate advanced container
          ...
          SearchFacetDialogHelper.updateDetailPageListItemsSelected(oDetailPageAttributeList, oAdvancedContainer); // update possible charts avr
            ...
            case "number":
              ...
              for (k = 1; k < oAdvancedConditionList.length; k++) {
                ...
                if (oAdvancedCheckBox.getSelected()) {
                  if (!isNaN(oAdvancedInputLeftValue) && // ToDo
                  !isNaN(oAdvancedInputRightValue) && // ToDo
                  oAdvancedInputRightValue >= oAdvancedInputLeftValue) {
                    ...
                    oConditionGroup = oFacetModel.sinaNext.createComplexCondition({
                      valueLabel: SearchFacetDialogHelper.oNumberFormat.format(oAdvancedInputLeftValue) + // ToDo
      ...
      value: function updateCountInfo(oDetailPage) {
        ...
        try {
          for (_iterator3.s(); !(_step3 = _iterator3.n()).done;) {
            ...
            if (typeof advancedCondition.getContent === "function"
            /* skip first item (list of ranges) */
            ) {
