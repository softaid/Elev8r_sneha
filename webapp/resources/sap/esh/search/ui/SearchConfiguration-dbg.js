...
(function(){
sap.ui.define(["sap/esh/search/ui/controls/SearchResultListSelectionHandler", "./controls/SearchResultListItemNote", "./SearchConfigurationSettings", "sap/base/Log"], function (SearchResultListSelectionHandler, __SearchResultListItemNote, __SearchConfigurationSettings, Log) {
  ...
  var SearchConfiguration = /*#__PURE__*/function (_SearchConfigurationS) {
    ...
    function SearchConfiguration(configuration) {
      ...
      _defineProperty(_assertThisInitialized(_this2), "log", Log.getLogger("sap.esh.search.ui.SearchConfiguration"));

      _defineProperty(_assertThisInitialized(_this2), "dataSourceConfigurations_Regexes", []);

      _defineProperty(_assertThisInitialized(_this2), "defaultDataSourceConfig", {});

      jQuery.extend(_assertThisInitialized(_this2), configuration);

      if (_this2.isUshell) {
        _this2.readUshellConfiguration();

        _this2.readOutdatedUshellConfiguration();
      }

      _this2.updateConfigFromUrlParameters();

      _this2.initDataSourceConfig();

      _this2.initDefaultDataSourceConfig();

      _this2.setModulePaths();

      _this2.setSinaLanguageDefault();
    ...
    _createClass(SearchConfiguration, [{
      ...
      key: "initDefaultDataSourceConfig",
      value: function initDefaultDataSourceConfig() {
      ...
      value: function updateConfigFromUrlParameters() {
        ...
        for (var parameterName in urlParameters) {
          ...
          var parameterType = this.getParameterType(parameterName); // ignore sina url parameters (these parameters are handled by sina itself, see sinaFactory)
