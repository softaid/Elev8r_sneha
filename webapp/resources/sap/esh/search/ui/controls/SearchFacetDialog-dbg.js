...
sap.ui.define(["../i18n", "sap/esh/search/ui/SearchHelper", "sap/esh/search/ui/SearchFacetDialogHelper", "sap/esh/search/ui/SearchFacetDialogHelper_SearchAdvancedCondition_ModuleLoader", "sap/esh/search/ui/SearchFacetDialogHelperCharts", "sap/esh/search/ui/controls/SearchAdvancedCondition", "sap/esh/search/ui/FacetItem", "sap/m/library", "sap/m/Page", "sap/m/Dialog", "sap/m/Select", "sap/m/CheckBox", "sap/m/SearchField", "sap/m/ToggleButton", "sap/m/Button", "sap/m/StandardListItem", "sap/m/SplitContainer", "sap/ui/core/Item", "sap/m/List", "sap/m/HBox", "sap/m/VBox", "sap/ui/model/Filter", "sap/ui/model/FilterOperator", "sap/m/ScrollContainer", "sap/m/Toolbar", "sap/m/IconTabBar", "sap/m/IconTabFilter", "sap/ui/model/json/JSONModel", "../sinaNexTS/providers/abap_odata/UserEventLogger", "../hierarchydynamic/SearchHierarchyDynamicFacet", "./SearchFacetHierarchyDynamic"], function (__i18n, SearchHelper, SearchFacetDialogHelper, SearchFacetDialogHelper_SearchAdvancedCondition_ModuleLoader, SearchFacetDialogHelperCharts, SearchAdvancedCondition, FacetItem, sap_m_library, Page, Dialog, Select, CheckBox, SearchField, ToggleButton, Button, StandardListItem, SplitContainer, Item, List, HBox, VBox, Filter, FilterOperator, ScrollContainer, Toolbar, IconTabBar, IconTabFilter, JSONModel, ___sinaNexTS_providers_abap_odata_UserEventLogger, __SearchHierarchyDynamicFacet, __SearchFacetHierarchyDynamic) {
  ...
  var SearchFacetDialog = Dialog.extend("sap.esh.search.ui.controls.SearchFacetDialog", {
    ...
    constructor: function _constructor(sId, settings) {
      ...
      if (jQuery["sap"].getUriParameters().mParams.newpie && // ToDo: jQuery
      jQuery["sap"].getUriParameters().mParams.newpie !== "false" // ToDo: jQuery
      ) {
        this.bOldPieChart = false;
      }
    ...
    resetAllFilters: function _resetAllFilters() {
      ...
      oModel.aFilters = []; // none of the above work since selections in all lists are physically counted to addFilterCondition
    ...
    createAttributeDetailPage: function _createAttributeDetailPage(sId, oContext) {
      ...
      var sDataType = oContext.getModel()["getAttributeDataType"](oContext.getModel().getProperty(oContext.getPath())); // create a settings container with select and checkBox, initial is not visible
      ...
      oSettings.setVisible(false); // create the attribute list for each facet
      ...
      oListContainer.setBusyIndicatorDelay(0); // create advanced search
      ...
      if (sDataType === "string" || sDataType === "text") {
        ...
        oAdvancedContainer.data("initial", true); // create a page for type string or text, content include settings container and attribute list, head toolbar has a setting button and a search field
        ...
        oListContainer.data("initial", true); // create a page for type number or date
    ...
    onDetailPageSelectionChange: function _onDetailPageSelectionChange(oEvent) {
      var oSelectedItem = oEvent.getParameter("listItem"); // update aFilters
      ...
      SearchFacetDialogHelper.updateCountInfo(oDetailPage); // deselect setting check box

      var oSettings = oList.getParent().getParent().getContent()[// ToDo
    ...
    onOkClick: function _onOkClick() {
      ...
      var aDetailPages = this.oSplitContainer.getDetailPages(); // no advanced filter
    ...
    resetIcons: function _resetIcons(oModel, sPath, oControl) {
      ...
      if (isTextDataType) {
        ...
        for (var i = 0; i < allDropdownbuttons.length; i++) {
          ...
          var asWhat = oControl.getProperty("tabBarItems")[oControl.chartOnDisplayIndex].getText(); // reset the main button tooltip
    ...
    onDetailPageSelectionChangeCharts: function _onDetailPageSelectionChangeCharts(oEvent) {
      ...
      if (oEvent.getSource && oEvent.getId() === "press") {
        ...
        becomesSelected = !isSelected; // first set the selected value in model

        oSelectedItem = oEvent.getSource();
        sSelectedBindingPath = oSelectedItem.getBindingContext().sPath + "/selected";
        model.setProperty(sSelectedBindingPath, becomesSelected); // update aFilters
        ...
        model = context.getModel();
        data = context.getObject();
        becomesSelected = oEvent.getId() === "selectData"; // first set the selected value in model
        ...
        oBindingObject.valueLabel = data.valueLabel; // update aItemsForBarChart
    ...
    updateDetailPageCharts: function _updateDetailPageCharts(aItems) {
      ...
      var contents = oListContainer.getContent(); // update pie chart (only for oPieChart.directUpdate ie old pie)
