...
sap.ui.define([
	...
	*/
	...
	CloudFilePicker.prototype._createNewFolderInline = function () {
		...
		oTableControl.insertItem(oNewFolderColumnListItem);
		setTimeout(function () {
	...
	CloudFilePicker.prototype._makeNewFolderEntry = function (sFolderName) {
		...
		oTableControl.removeItem(oNewFolderColumnListItem);
		oNewFolderColumnListItem.destroy();
		...
		oPromise.then(function(bSuccess) {
			if (!bSuccess) {
				...
				oTableControl.setSelectedItem(oTableControl.getItems().length > 0 ? oTableControl.getItems()[0] : null);
				this.setBusy(false);
			}
		}.bind(this));
	};
					...
					if (index == 0) {
						...
						var oCrumb = new Link({
							...
							press: function (oEvent) {
								if (oNewFolderColumnListItem && oTableControl.indexOfItem(oNewFolderColumnListItem) > -1) {
									oTableControl.removeItem(oNewFolderColumnListItem);
									oNewFolderColumnListItem.destroy();
								}
	...
	CloudFilePicker.prototype._loadFileShareRootFolder = function (sFileShareKey) {
		...
		oTableControl.setBindingContext(oContext);
	};
		...
		oConfirmationButton = new Button({
			...
			press: function () {
				...
				if ((this.getFilePickerMode() === FilePickerModes.FileOnly) || sCurrentItemInInput) {
					if (this.getEnableDuplicateCheck() && this._checkForDuplicate(oTableControl, sCurrentItemInInput.toLowerCase())) {
	...
	CloudFilePicker.prototype._setConfirmationButtonEnabled = function (bEnabled, oSelectedFolderInfo, bFileNameEdit) {
		...
		if (oFileNameControl && oFileNameControl.getValue()) {
			bIsFileNameExist = oFileNameControl.getValue() !== '';
		}
	...
	CloudFilePicker.prototype._closeDialog = function (bReplaceExistingFile) {
		...
		var oSelectedItem = oTableControl.getSelectedItem();
		if (oSelectedItem) {
