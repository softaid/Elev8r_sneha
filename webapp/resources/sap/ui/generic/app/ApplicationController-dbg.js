...
sap.ui.define([
	...
	ApplicationController.prototype._onValidateFieldGroup = function (aGroups, oView) {
		var i, len = aGroups.length, fRequest, mRequests = {
			...
			aRequests: [],
			bHasTargets: true
		}, aValiditiyPromises = [], that = this;
		// calculate the requests to be triggered.
		//  the return response of _executeFieldGroup is transfered to promises that get resolved in order to wait for all promises to be settled before triggering the submit 
		for (i = 0; i < len; i++) {
			aValiditiyPromises.push(this._executeFieldGroup(aGroups[i], mRequests, oView).then(
				function() {
					return true;
				},
				function() {
		...
		return Promise.all(aValiditiyPromises).then(fnValidityPromisesCallback);
	};
	 ...
	 * @param {object} oView Reference to the view
	 * @returns {Promise} <code>Promise</code> for validility check that is resolved when their are no errors and rejected if any error found.
	 * @private
	 */
	 ...
	 * or entities are passed then the global side effect (the one without source properties and source entites) will be executed.
