...
sap.ui.define([
		...
		aContexts.forEach(function(oSingleContext) {
			...
			if (this._oFunctionImport.parameter) {
				...
				for (var i = 0; i < this._oFunctionImport.parameter.length; i++) {
					...
					mActionParams.parameterData[sParameterName] = (mAdditionalParmData && mAdditionalParmData[sParameterName]) || oParameterValue;
	...
	ActionUtil.prototype._triggerActionPromise = function (oFuncHandles, aEntityContext, mActionParam, oActionContexts, bIsStrict) {
		var that = this;
		...
		var _dataObject = oActionContexts[0].getObject();
		...
		mActionParam.additionalParameters.forEach(function (param) {
			var _dataObjectValue = _dataObject[param.name];
			...
			oActionContexts.forEach(function (ctx) {
		...
		aEntityContext.forEach(function (ctx, i) {
			that.getApplicationController().submitActionContext(ctx, oActionContexts[i], sFunctionImportName);
		});
