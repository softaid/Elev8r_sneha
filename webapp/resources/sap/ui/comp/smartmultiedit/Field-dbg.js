...
sap.ui.define([
	 ...
	 * @private
	 */
	Field.prototype._calculateTextAbsolutePath = function (oValue) {
		if (this._oAnnotations.valueListData &&
			this._oSmartField._oFactory._oMetaData &&
			this._oSmartField._oFactory._oMetaData.property) {
			var sValueListEntitySet = this._oAnnotations.valueListData.valueListEntitySetName;
			return this._oSmartField._oFactory._oHelper.getAbsolutePropertyPathToValueListEntity({
				entityID: oValue,
				entitySet: { name: sValueListEntitySet},
				property: this._oSmartField._oFactory._oMetaData.property.valueListKeyProperty
				});
		}
		return '';
	};
	...
	Field.prototype._addInnerSelectItem = function (oValue, oContext) {
		...
		if (this.isComboBox()) {
			...
			if (sRecordTextPath) {
				sText = oContext.getObject(sRecordTextPath);
			} else {
				// get the absolute path to get the text of the selected key
				var sAbsolutePath = this._calculateTextAbsolutePath(oValue);
				sText = sAbsolutePath ? this.getModel().getProperty(sAbsolutePath) : oValue;
			}
