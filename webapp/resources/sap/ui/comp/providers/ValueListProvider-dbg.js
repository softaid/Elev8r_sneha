...
sap.ui.define([
	...
	ValueListProvider.prototype._onAnnotationLoad = function (mValueList) {
		...
		if (this.sContext === "SmartFilterBar" && this._hasNotValidatedSingleField(this.sFieldName)) {
			this._validateInput(this.oFilterProvider._oNotValidatedSingleFields[this.sFieldName]);
		...
		this._oTemplate = new ListItem({
			...
			text: {
				...
				formatter: function(sKey, sDescription) {
					var fnFormatter = FormatUtil.getFormatterFunctionFromDisplayBehaviour(this.sDDLBDisplayBehaviour),
	...
	ValueListProvider.prototype._validateInput = function (sInput, fAsyncCallback) {
		...
		if (oControl.__sValidationText !== sInput) {
			...
			if (sInput === this._truncateSearchText(sInput) || (this.sContext === "SmartFilterBar" && this._fieldViewMetadata.filterRestriction === "single")) {
						...
						success: function (oResponseData) {
							...
							if (oResponseData) {
								...
								if (oResponseData.results && oResponseData.results.length >= 1) {
									...
									if (this.sContext === "SmartFilterBar" && this._fieldViewMetadata.filterRestriction === "single" && oResponseData.results.length > 1) {
	...
	ValueListProvider.prototype._getDistinctSuggestions = function (aData) {
		var oUnique = {},
			aDistinct = [],
			aKeysToRemove = ["__metadata", HistoryConstants.getSuggestionsGroupPropertyName()];

		aData.forEach(function (x) {
			var aItemKeys = Object.keys(x),
				bEveryEntityKeyExistInObject = this._aKeys && this._aKeys.every(function (sKey) { return aItemKeys.includes(sKey); }),
				aKeysToInclude = bEveryEntityKeyExistInObject ? this._aKeys : aItemKeys;

			var aKeyFields = Object.entries(x).filter(function (aEntry) {
				var sPropertyName = aEntry[0];
				return aKeysToInclude.includes(sPropertyName) && !aKeysToRemove.includes(sPropertyName);
			}, this);

			var sKey = aKeyFields.toString();
