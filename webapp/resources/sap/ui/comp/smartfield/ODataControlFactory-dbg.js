...
sap.ui.define([
	...
	ODataControlFactory.prototype._createEdmDisplay = function(oSettings) {
		var oInnerControl,
			...
			bIgnoreComboBox = ((oConfig && (oConfig.isInnerControl !== true)) || this._oParent.isContextTable()),
			oControlSelectorConfig = this._oSelector.checkComboBox({mode: sMode, configOnly: bIgnoreComboBox}),
				...
				if (bObjectIdentifier) {
					...
					mAttributes.text.formatter = function(sId, sDescription) {
						...
						if (oControlSelectorConfig.combobox) {
							 var fnFormatter = that._formatDisplayBehaviour("defaultComboBoxReadOnlyDisplayBehaviour", sId, sDescription),
							 sItemText = fnFormatter(sId, sDescription);
		...
		if (bObjectIdentifier) {
			...
			if (typeof mAttributes.text.formatter === "undefined" && !sTimeZonePath) {
			...
			bWasDisplayCombobox = this._oSelector._checkComboBox({mode: sMode, configOnly: bIgnoreComboBox}).combobox && (this._oParent.getFetchValueListReadOnly() || (vTextAnnotationPath === undefined));
			// Because of backwards compatibility,
			// the SmartField.getValue should return values without any description,
			// e.g. the value of ComboBox.getSelectedKey
			if (bWasDisplayCombobox){
				getValue = null;
			}
		}
	...
	ODataControlFactory.prototype._createEdmNumeric = function(oSettings) {
		var oControlSelector = this._oSelector.checkComboBox(oSettings), sMode = oSettings ? oSettings.mode : undefined, oInputField;
	...
	ODataControlFactory.prototype._checkTextInDisplayModeValueList = function (oSettings) {
		...
		return !!(
			...
			this._getDisplayBehaviourConfiguration("defaultInputFieldDisplayBehaviour") &&
			...
			(this._oHelper.oAnnotation.getTextArrangement(oEdmProperty) || this._oParent.data("defaultInputFieldDisplayBehaviour")) &&
			// this._oMetaData.annotations && this._oMetaData.annotations.valuelistType !== "fixed-values" && // We are in a value list scenario
	...
	ODataControlFactory.prototype.isValueInitialWithTextArrangement = function (oEdmProperty) {
		return this._oParent._isValueInitial() && this._oHelper.oAnnotation.getText(oEdmProperty);
	};
