...
sap.ui.define([
	...
	SmartField.prototype.init = function() {
		...
		this.attachModelContextChange(function () {
			...
			if (this._hasDefaultModel()) {
				if (this.getBindingContext()) {
					this._createICRenderedDeferred();
				}
		...
		// TODO: Remove after stakeholder confirms it's no longer used.
		this._innerControlsRendered = Promise.resolve();
		// Initial value flag
	 ...
	 * @private
	 */
	SmartField.prototype._getICRenderedPromise = function () {
	 ...
	 * @private
	 */
	SmartField.prototype._createICRenderedDeferred = function () {
		var oDef;
	...
	SmartField.prototype.setTextInEditModeSource = function(sTextInEditModeSource) {
		...
		this.setProperty("textInEditModeSource", sTextInEditModeSource);
	 ...
	 * @param {string} sMode The mode of the control. In context of this method this is the preferred setting ot be used.
	 * @private
	 */
	SmartField.prototype._propagateToInnerControls = function (sMode) {
		if (sMode === "edit") {
	...
	SmartField.prototype._onInnerControlToggled = function(oSettings) {
		...
		// Lifecycle hook
		this._propagateToInnerControls(oSettings.mode);
	 ...
	 * @private
	 */
	SmartField.prototype._getComputedTextInEditModeSource = function (bExact) {
	...
	SmartField.prototype._calculateCurrentState = function() {
		var bClientSideMandatoryCheck = this.getClientSideMandatoryCheck(),
			...
			bDestroyFactory = bValueBindingPathChanged || bModelChanged || bEntitySetChanged,
			bIsFactoryJustCreated = bDestroyFactory,
			bForceTextArrangementFetch = bBindingContextChanged || !bModeInitialized || bModeChanged;
	...
	SmartField.prototype.enhanceAccessibilityState = function(oElement, mAriaProps) {
		...
		if (oParent && oParent.enhanceAccessibilityState) {
			// use SmartField as control, but aria properties of rendered inner control.
	...
	SmartField.prototype._handleAriaLabelledByInSmartTable = function(oUoMNestedSmartField) {
		var bAlreadyExistingAriaLabelledByAssociation,
		that = this;
		...
		if (this.getAriaLabelledBy().length > 0) {
			this.getInnerControls().forEach(function(oControl) {
				...
				if (oControl.addAriaLabelledBy && oControl.getAriaLabelledBy) {
					bAlreadyExistingAriaLabelledByAssociation = oControl.getAriaLabelledBy().indexOf(that.getAriaLabelledBy()[0]) > -1;

					if (!bAlreadyExistingAriaLabelledByAssociation &&
	...
	SmartField.prototype._handleUoMAriaLabelledBy = function(oUoMNestedSmartField) {
		this.getInnerControls().forEach(function(oControl) {
			...
			if (oControl.addAriaLabelledBy && oControl.getAriaLabelledBy && !oUoMNestedSmartField.getEditable()){
				var bAlreadyExistingAriaLabelledByAssociation = oControl.getAriaLabelledBy().indexOf(oUoMNestedSmartField.getId() + "-text") > -1;
				if (!bAlreadyExistingAriaLabelledByAssociation) {
					oControl.addAriaLabelledBy(oUoMNestedSmartField.getId() + "-text");
				}
			}
		});
	};
	 ...
	 */
	 SmartField.prototype._isValueInitial = function () {
		...
		if (this._isValueInValidation()) {
			return this._getValueInValidation() === oDataState.getOriginalValue();
		}

		return oDataState.getValue() === oDataState.getOriginalValue();
	};
	...
	SmartField.prototype._initState = function () {
		return Object.create({
			...
			isModeInitialized: function (sMode) {
				var sValueProperty = sMode.toUpperCase(),
	 ...
	 * @private
	 * @ui5-restricted sap.ui.rta SmartField.designtime.js
	 * @return {Promise}
	 */
	 ...
	 * @private
	 * @UI5-restricted sap.ui.generic.template.ObjectPage
	 */
