...
sap.ui.define([
	...
	Viewport.prototype.init = function() {
		...
		this.raster = {
			...
			nodesThreshold: 10000
		};

		this.raster.vectorImage.onload = function() {
			// Rasterize vector image in canvas
			var ctx = this.raster.bmpCanvas.getContext("2d");
			ctx.clearRect(0, 0, this.raster.bmpCanvas.width, this.raster.bmpCanvas.height);
			ctx.drawImage(this.raster.vectorImage, 0, 0);

			// Now get raster image from canvas and store it for later rendering
			this.raster.rasterImage.width = this.raster.bmpCanvas.width;
			this.raster.rasterImage.height = this.raster.bmpCanvas.height;
			this.raster.rasterImage.src = this.raster.bmpCanvas.toDataURL();
		}.bind(this);
	...
	Viewport.prototype._handleResize = function(event) {
		...
		if ((oldWidth === 0 || oldHeight === 0) && !this._camera._initialPosition) {
	...
	Viewport.prototype._updateRasterImage = function() {
		...
		this.raster.bmpCanvas.width = this._width * devicePixelRatio;
		this.raster.bmpCanvas.height = this._height * devicePixelRatio;
		this.raster.bmpCanvas.style.width = this._width + "px";
		this.raster.bmpCanvas.style.height = this._height + "px";

		// Save current viewBox, we'll use it later for canvas pan/zoom
		this.raster.bmpCanvas.viewBox = this._getViewBox();

		if (sap.ui.Device.browser.firefox) {
			// Unfortunately FF can't render SVG to canvas if SVG has width and height in units of percentage
			this._svgElement.setAttribute("width", this.raster.bmpCanvas.width + "px");
			this._svgElement.setAttribute("height", this.raster.bmpCanvas.height + "px");
		}
		...
		this.raster.vectorImage.width = this.raster.bmpCanvas.width;
		this.raster.vectorImage.height = this.raster.bmpCanvas.height;
		this.raster.vectorImage.src = data;
	};

	Viewport.prototype._startRasterMode = function(delayEnd) {
		if (this.raster.timer) {
			// Reset raster delay timer
			clearTimeout(this.raster.timer);
			this.raster.timer = null;
		}
	...
	Viewport.prototype._endRasterMode = function() {
		...
		if (this.raster.delayEnd) {
			this.raster.timer = setTimeout(function() {
				this.raster.timer = null;
				clearRasterMode();
			}.bind(this), 1000);
		} else {
	...
	Viewport.prototype._bmpDraw = function() {
		...
		var ctx = this.raster.bmpCanvas.getContext("2d");
		ctx.clearRect(0, 0, this.raster.bmpCanvas.width, this.raster.bmpCanvas.height);
		if (this.raster.bmpCanvas.viewBox) {
			var oldPos = this._camera._worldToScreen(this.raster.bmpCanvas.viewBox[0], this.raster.bmpCanvas.viewBox[1]);
			...
			var scale = this.raster.bmpCanvas.viewBox[3] / viewBox[3];
			ctx.scale(scale, scale);
		}
		ctx.drawImage(this.raster.rasterImage, 0, 0);
		this.raster.bmpCanvas.viewBox = viewBox;
	};
	...
	Viewport.prototype._updateViewBox = function() {
		...
		if (this.raster.rasterMode) {
			window.requestAnimationFrame(this._bmpDraw.bind(this));
		}
