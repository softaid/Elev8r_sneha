...
sap.ui.define([
	...
	AxisAngleRotationToolGizmo.prototype.init = function() {
		...
		function createTouchCircle(axis, radius, segments) {
			var geometry = new THREE.TorusBufferGeometry(radius, 16 / 96, 4, segments, axis === 2 ? Math.PI : math2PI);
			if (axis === 0) {
		...
		for (i = 0; i < 3; i++) {// add axis title touch area
			var sphere = new THREE.Mesh(new THREE.IcosahedronBufferGeometry(0.25, 0), new THREE.MeshBasicMaterial({ opacity: 0.2, transparent: true }));
	...
	AxisAngleRotationToolGizmo.prototype._createGizmoObject = function() {
		...
		function createGizmoCircle(axis, color, radius, segments) {
			var geometry = new THREE.TorusBufferGeometry(radius, 1 / 96, 4, segments, axis === 2 ? Math.PI : math2PI);
			if (axis === 0) {
				geometry.rotateY(mathHalfPI);
			} else if (axis === 1) {
				geometry.rotateX(mathHalfPI);
		...
		userData.arcMeshes = [];
		for (i = 0; i < 3; i++) {
			var arcMesh = new THREE.Mesh(new THREE.Geometry(), new THREE.MeshBasicMaterial({ vertexColors: THREE.VertexColors, opacity: 0.5, transparent: true, side: THREE.DoubleSide, depthWrite: false }));
	...
	function updateArcMesh(arcMesh, axisIndex, angle1, angle2) {
		...
		arcMesh.geometry = geom;
		arcMesh.geometry.setAttribute("color", new THREE.Float32BufferAttribute(colors, 3));
	...
	AxisAngleRotationToolGizmo.prototype._updateNodeRotation = function(nodeInfo) {
		_axis.fromArray(azimuthElevationToAxis(nodeInfo.azimuth, nodeInfo.elevation));
		_quaternion.setFromAxisAngle(_axis, degToRad(nodeInfo.angle));
		nodeInfo.node.quaternion.multiplyQuaternions(_quaternion, nodeInfo.quaternion);
	...
	AxisAngleRotationToolGizmo.prototype._rotate = function(euler) {
		...
		this._nodes.forEach(function(nodeInfo) {
			...
			nodeInfo.elevation = THREE.Math.clamp(nodeInfo.beginElevation + this._rotationDelta.z, -90, 90);
	...
	AxisAngleRotationToolGizmo.prototype.setValues = function(values) {
		values.forEach(function(value) {
			...
			if (nodeInfo) {
				...
				_axis.fromArray(azimuthElevationToAxis(nodeInfo.azimuth, nodeInfo.elevation));
				_quaternion.setFromAxisAngle(_axis, -degToRad(nodeInfo.angle));
				nodeInfo.quaternion.premultiply(_quaternion);
				this._updateNodeRotation(nodeInfo);
			}
		}, this);
		this._updateEditValue();
	...
	AxisAngleRotationToolGizmo.prototype._updateGizmoTransformation = function(i, camera) {
		...
		gizmo.userData.axisTitles.children.forEach(function(child) {
			child.quaternion.copy(gizmo.quaternion).inverse().multiply(camera.quaternion);
		});
		...
		for (var vi = 0; vi < 3; vi++) {
			...
			valueMesh.quaternion.copy(gizmo.quaternion).inverse().multiply(camera.quaternion);
