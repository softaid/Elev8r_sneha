...
sap.ui.define([
		...
		for (var ni = 0, nc = this._nodes.length; ni < nc; ni++) {
			...
			if (!nodeInfo.ignore) {
				...
				if (userData && userData.euler && userData.eulerInParentCoors) {
					...
					this._rotationDelta.set(THREE.Math.radToDeg(x - userData.eulerInParentCoors.x), THREE.Math.radToDeg(y - userData.eulerInParentCoors.y), THREE.Math.radToDeg(z - userData.eulerInParentCoors.z));
		...
		for (var ni = 0, nc = this._nodes.length; ni < nc; ni++) {
			...
			if (!nodeInfo.ignore) {
				...
				if (userData && userData.euler && userData.eulerInParentCoors) {
					...
					this._rotationDelta.set(THREE.Math.radToDeg(x - userData.euler.x), THREE.Math.radToDeg(y - userData.euler.y), THREE.Math.radToDeg(z - userData.euler.z));
					...
					if (this._coordinateSystem === CoordinateSystem.Local) {
						...
						currentMatInv = currentMatInv.getInverse(currentMat);
					}
				...
				if (userData && userData.euler) {
					...
					if (this._playback) {
						...
						if (sequenceOffset) {
							...
							em.multiply(new THREE.Matrix4().getInverse(sm));
						}
					}
	...
	RotateToolGizmo.prototype._rotate = function(euler) {
		this._rotationDelta.set(THREE.Math.radToDeg(euler.x), THREE.Math.radToDeg(euler.y), THREE.Math.radToDeg(euler.z));
	...
	RotateToolGizmo.prototype._setRotationAxisAngle = function(axisIndex, angle1, angle2) {
		...
		var nodesProperties = this._getNodesProperties();
		if (this._tool.fireEvent("rotating", { x: THREE.Math.radToDeg(euler.x), y: THREE.Math.radToDeg(euler.y), z: THREE.Math.radToDeg(euler.z), nodesProperties: nodesProperties }, true)) {
			this._printEventInfo("Event 'rotating'", THREE.Math.radToDeg(euler.x), THREE.Math.radToDeg(euler.y), THREE.Math.radToDeg(euler.z), nodesProperties);
			this._rotate(euler);
	...
	RotateToolGizmo.prototype.setValue = function(value) {
		if (this._gizmoIndex >= 0 && this._handleIndex >= 0 && this._handleIndex < 3) {
			var euler = new THREE.Euler();
			euler[["x", "y", "z"][this._handleIndex]] = THREE.Math.degToRad(value - this._value.getComponent(this._handleIndex));
			this.beginGesture();
			this._rotate(euler);
			this.endGesture();
		}
