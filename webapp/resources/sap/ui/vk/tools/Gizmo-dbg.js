...
sap.ui.define([
	...
	Gizmo.prototype._getOffsetForRestTransformation = function(node) {
		if (this._viewport._viewStateManager) {
			...
			userData.matRestInv = new THREE.Matrix4().getInverse(userData.matRest);
			...
			userData.matInitialInv = new THREE.Matrix4().getInverse(node.matrix);
			userData.quatInitialDiff = userData.initialQuaternion.clone().multiply(userData.quatRest.clone().inverse());
			userData.quatInitialDiffInv = userData.quatInitialDiff.clone().inverse();
	...
	Gizmo.prototype._updateEditingForm = function(active, axisIndex, label) {
		...
		if (domRef) {
			if (active && this._tool && this._tool.getShowEditingUI()) {
				...
				var x = THREE.Math.clamp(viewportRect.width * (position.x * 0.5 + 0.5) + (alignRight ? -20 : 20), Math.max(dx, 0), viewportRect.width + Math.min(dx, 0));
