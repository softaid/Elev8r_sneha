		...
		constructor: function(viewport) {
			...
			var ThreeJsCameraController = function(vp) {
				...
				this.beginGesture = function(x, y) {
					...
					var hit = camera.userData.isRedlineActivated ? null : viewport.hitTest(x, y, sceneRef, camera);
					if (hit) {
						...
						sceneRef._expandBoundingBox(boundingBox, true, true);
				...
				this.rotate = function(dx, dy, isTurnTableMode) {
					...
					if (this.isTurnTableMode) {
						...
						deltaPitch = THREE.Math.clamp(deltaPitch, MIN_PITCH - pitch, MAX_PITCH - pitch);
					} else {
				...
				this.zoom = function(zoomFactorScaler) {
					...
					if (!camera.userData.isRedlineActivated) {
						...
						viewport.getScene().getSceneRef()._expandBoundingBox(boundingBox, true, ignoreDynamicObjects);
				...
				this.animateCameraUpdate = function() {
					...
					if (camera.view && camera.view.enabled) { // Redline mode is activated
						...
						camera.view.offsetX = THREE.Math.lerp(this._oldCamera.view.offsetX, this._newCamera.view.offsetX, interpolateCoeff);
						camera.view.offsetY = THREE.Math.lerp(this._oldCamera.view.offsetY, this._newCamera.view.offsetY, interpolateCoeff);
						camera.view.width = THREE.Math.lerp(this._oldCamera.view.width, this._newCamera.view.width, interpolateCoeff);
						camera.view.height = THREE.Math.lerp(this._oldCamera.view.height, this._newCamera.view.height, interpolateCoeff);
					...
					if (camera.isOrthographicCamera && this._newCamera.isOrthographicCamera && this._oldCamera.isOrthographicCamera) {
						...
						if (this._transitionEffect === "zoomIn") {
							...
							camera.zoom = interpolateCoeff < 0.5 ?
								THREE.Math.lerp(this._oldCamera.zoom, zoomInZoom, interpolateCoeff * 2) :
								THREE.Math.lerp(zoomInZoom, this._newCamera.zoom, (interpolateCoeff - 0.5) * 2);
						} else {
					...
					if (camera.isPerspectiveCamera && this._newCamera.isPerspectiveCamera && this._oldCamera.isPerspectiveCamera) {
						if (this._transitionEffect === "zoomIn") {
							...
							camera.fov = interpolateCoeff < 0.5 ?
								THREE.Math.lerp(this._oldCamera.fov, zoomInFOV, interpolateCoeff * 2) :
								THREE.Math.lerp(zoomInFOV, this._newCamera.fov, (interpolateCoeff - 0.5) * 2);
						} else {
				...
				this.zoomObject = function(nodeRef, isIn, timeInterval) {
					...
					var boundingBox = new THREE.Box3();
					(isIn && nodeRef ? nodeRef : viewport.getScene().getSceneRef())._expandBoundingBox(boundingBox, true, true);
