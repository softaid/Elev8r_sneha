...
sap.ui.define([
	...
	Viewport.prototype.setScene = function(scene) {
		if (this._scene) {
			...
			nh.detachChanged(this._clearAnnotations, this);
			nh.detachNodeCreated(this._nodeCreatedHandler, this);
		...
		if (this._scene) {
			this._scene.getDefaultNodeHierarchy().attachChanged(this._clearAnnotations, this);
			this._scene.getDefaultNodeHierarchy().attachNodeCreated(this._nodeCreatedHandler, this);
		}
	...
	Viewport.prototype._clearAnnotations = function(clearAll) {
		...
		this._annotationsLoaded = false;
	};

	Viewport.prototype._nodeCreatedHandler = function(evt) {
		...
		if (nodeRef._vkGetNodeContentType() == NodeContentType.Annotation) {
			...
			if (view) {
				view.getNodeInfos().push({ target: nodeRef, visible: true });
			}
		}
	};
	...
	Viewport.prototype._renderLoop = function() {
		...
		var vsm = this._getViewStateManagerThreeJS();
		if (vsm) {
			this._shouldRenderFrame |= vsm._playTransition();
			this._shouldRenderFrame |= vsm._playHighlight();
		}
	...
	Viewport.prototype._updateWorldMatrixRecursive = function(node, force) {
		if (node.visible === false) {// ignore invisible objects
			return;
		}
		...
		for (var i = 0, l = children.length; i < l; i++) {
			this._updateWorldMatrixRecursive(children[i], force);
		}
	};
	...
	Viewport.prototype.render = function() {
		...
		try {
			...
			this._updateWorldMatrixRecursive(nativeScene, false);
			nativeScene.autoUpdate = false;
			...
			if (this._camera.getUsingDefaultClipPlanes() || (nativeCamera.isOrthographicCamera && nativeCamera.zoom <= 0)) {
				boundingBox = this._scene._computeBoundingBox(true, false, true);
	...
	Viewport.prototype.getMaterialImage = function(material, width, height, topColor, bottomColor, margin) {
		width = THREE.Math.clamp(width || 256, 8, 2048);
		height = THREE.Math.clamp(height || 256, 8, 2048);
				...
				if (content.builders && content.builders.length > 0) { // the content was created by sap.ui.vk.threejs.SceneBuilder, but has no default camera
					...
					if (!boundingBox.isEmpty()) {
						...
						if (content.upAxis === 4) { // z-up
							euler = new THREE.Euler(THREE.Math.degToRad(90 - 30), 0, THREE.Math.degToRad(55 - 90), "ZXY");
						} else { // y-up
							euler = new THREE.Euler(THREE.Math.degToRad(-30), THREE.Math.degToRad(55 - 90), 0, "YXZ");
						}
	...
	Viewport.prototype.resetCurrentView = function() {
		...
		this._getViewStateManagerThreeJS()._resetNodesStatusByCurrentView(this._currentView, true, true);
	...
	Viewport.prototype.getAnnotations = function() {
		if (this.getScene() == null || this._annotationsLoaded) {
			return this.getAggregation("annotations") || [];
		}

		// If scene does not contain any Html annotation then we have nothing to do
		if (this.getScene().annotations) {
			var viewAnnotationNodes = [];
			var view = this.getCurrentView();
			if (view) {
				// Find all annotation nodes in current view
				view.getNodeInfos().forEach(function(viewNode) {
					if (viewNode.target._vkGetNodeContentType() === NodeContentType.Annotation) {
						viewAnnotationNodes.push(viewNode);
					}
				});
			}

			this.getScene().annotations.forEach(function(annotationInfo) {
				var annotation = null;
				if (view) {
					// If we have current view then create only Html annotations that exist in that view
					for (var n = 0; n < viewAnnotationNodes.length; n++) {
						var node = viewAnnotationNodes[n];
						if (node.target._vkPersistentId() === annotationInfo.node._vkPersistentId() &&
							(node.annotationId === annotationInfo.annotation.id || node.annotationId === undefined)) {
							// This annotation is in current view, or associated with a newly created node, create it
							annotationInfo.node.visible = node.visible;
							annotation = Annotation.createAnnotation(annotationInfo, this);
					...
					// Otherwise create all Html annotations in scene
					annotation = Annotation.createAnnotation(annotationInfo, this);
					annotation.autoGenerated = true;
					this.addAnnotation(annotation);
				}
				if (annotation) {
					// If Html annotation is created then set it's visibility to it's node visibility state
					var visible = this._viewStateManager.getVisibilityState(annotationInfo.node);
					annotation.setDisplay(visible);
				}
			}.bind(this));

			// Set animation delays for each annotation to play one after another, not all at once
			var allAnnotations = this.getAggregation("annotations") || [];
			var delay = 0;
			allAnnotations.forEach(function(annotation) {
				if (annotation.getAnimate() && annotation.getAnimationDelay() === -1) {
