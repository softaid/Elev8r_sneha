...
sap.ui.define([
	...
	Billboard.prototype._planeGeometry = new THREE.PlaneBufferGeometry(1, 1);
	...
	Billboard.prototype._setBillboardTexture = function(canvas, width, height) {
		...
		this._billboard.geometry = new THREE.PlaneBufferGeometry(1, 1).setAttribute("uv", new THREE.Float32BufferAttribute([0, 1, u, 1, 0, 1 - v, u, 1 - v], 2));
	...
	Billboard.prototype._update = function(renderer, camera, viewportSize) {
		...
		if (coordinateSpace === BillboardCoordinateSpace.Screen) {// image note
			var dist = THREE.Math.lerp(camera.near, camera.far, 1e-4); // as close as possible to the near clipping plane
	...
	Billboard.prototype._ortho2DUpdate = function(renderer, camera) {
		...
		this.matrix.getInverse(this.parent.matrixWorld).multiply(this.matrixWorld);
