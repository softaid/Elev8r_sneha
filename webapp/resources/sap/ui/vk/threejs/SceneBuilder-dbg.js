...
sap.ui.define([
	...
	SceneBuilder.prototype.hasAnnotation = function(annotationId) {
		return this._annotations.has(annotationId);
	};
	...
	SceneBuilder.prototype.setAnnotationPersistentId = function(annotation, sid, sceneId) {
		this._resetCurrentScene(sceneId);

		if (this._annotations) {
			var annotationInfo = this._annotations.get(annotation.id);
			if (annotationInfo) {
				this._annotations.delete(annotation.id);
				annotationInfo.annotation.id = sid;
				this._annotations.set(sid, annotationInfo);
				return true;
			}
		}
		return false;
	...
	SceneBuilder.prototype.removeNode = function(nodeRef) {
		...
		// Check if node is annotation node and if found then delete annotation
		this._annotations.forEach(function(annotationInfo) {
			if (annotationInfo.node === nodeRef) {
				this._annotations.delete(annotationInfo.annotation.id);
			}
		}.bind(this));
	...
	SceneBuilder.prototype.insertSubmesh = function(submeshInfo, sceneBoundingbox) {
		...
		if (submeshInfo.lods) {   // streaming with lods
			...
			if (geometry) {
				...
				var useMarchingCubes = true;
				var boundingBox = lod.boundingBox;
				if (boundingBox && sceneBoundingbox) {
					var xRatio = (boundingBox[3] - boundingBox[0]) / (sceneBoundingbox[3] - sceneBoundingbox[0]);
					var yRatio = (boundingBox[4] - boundingBox[1]) / (sceneBoundingbox[4] - sceneBoundingbox[1]);
					var zRatio = (boundingBox[5] - boundingBox[2]) / (sceneBoundingbox[5] - sceneBoundingbox[2]);
					useMarchingCubes =
						xRatio > this._voxelThreshold ||
						yRatio > this._voxelThreshold ||
						zRatio > this._voxelThreshold;
				}

				var innerBoxGeometry;
				var innerBoxLod = useMarchingCubes && findInnerBoxLOD(submeshInfo.lods);
				...
				if (boundingBox && boundingBox.length === 6) {
					...
					submesh.geometry.applyMatrix4(mat);
				}
		...
		for (var i = 0; i < children.length; i++) {
			...
			if (geometryId && child.userData.geometryId === geometryId && child.geometry !== geometry) {
				if (child.type === "Mesh" && !geometry.isPolyline) {
					...
					if (geometry.isPositionQuantized && !(child.parent && child.parent.userData.transformType === "ORTHO2D")) {
	...
	SceneBuilder.prototype.setGeometry = function(geoInfo) {
		...
		var indexAttribute = new THREE.BufferAttribute(new Uint16Array(data.indices), 1);
		var positionAttribute = new THREE.BufferAttribute(new Float32Array(data.points), 3);

		geometry.setIndex(indexAttribute);
		geometry.setAttribute("position", positionAttribute);

		if (!geoInfo.isPolyline) {
			if (!data.normals || data.normals.length !== positionAttribute.array) {
				data.normals = computeNormals(positionAttribute.array, indexAttribute.array);
			}
			var normalAttribute = new THREE.BufferAttribute(new Float32Array(data.normals), 3);
			geometry.setAttribute("normal", normalAttribute);

			if (data.uvs && data.uvs.length * 3 === data.points.length * 2) {
				geometry.setAttribute("uv", new THREE.BufferAttribute(new Float32Array(geoInfo.data.uvs), 2));
			}
		} else {
	...
	SceneBuilder.prototype.createAnnotation = function(annotation, sceneId) {
		...
		if (type === "html") {
			...
			this._annotations.set(annotation.id, {
	...
	SceneBuilder.prototype.createCamera = function(cameraInfo, sceneId) {
		...
		if (cameraInfo.ortho) {
			...
			nativeCamera = new THREE.PerspectiveCamera(THREE.Math.radToDeg(cameraInfo.fov), 1, near, far);
		}
	...
	SceneBuilder.prototype.createMaterial = function(materialInfo) {
		...
		// Empirical approximation of shininess based on glossiness and specular level
		...
		function updateMaterialTexture(m, texture) {
			switch (type) {
				...
				case TextureType.Decal:
					...
					m.transparent |= imageDataUri.startsWith("data:image/png");
					break;
