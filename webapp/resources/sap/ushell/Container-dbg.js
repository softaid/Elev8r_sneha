...
sap.ui.define([
    ...
    var S_COMPONENT_NAME = "sap.ushell.services.Container",
        ...
        aAsyncLogoutEventFunctions = []; //asyc logout even functions that return a promise
    ...
    function Container (oAdapter) {
         ...
         * Besides UICompoents a control (i.e. extend <code>sap.ui.core.Control</code>) is accepted, too.
         *
        ...
        this.createRenderer = function (sRendererName, bAsync) {
            ...
            function getRendererInstance (Module) {
                ...
                var oShellControl;
                var UIComponent = sap.ui.require("sap/ui/core/UIComponent"); // UIComponent has been loaded implicitly already when Renderer is a UIComponent
            ...
            function getRendererInstanceAsync (Module) {
                return new Promise(function (resolve, reject) {
                    sap.ui.require([
                        ...
                        "sap/ui/core/routing/Router" // As fiori2.Renderer does not use manifst-first the router needs to be loaded async beforehand
        ...
        */
        ...
        this._getService = function (sServiceName, sParameter, bAsync) {
            ...
            if (!mServicesByName.containsKey(sKey)) {
                ...
                // Reserved for existing applications that use Container API to get a service:
                Service = sap.ui.requireSync(sModuleName.replace(/[.]/g, "/")); // LEGACY API (deprecated)
        ...
        this.defaultLogout = function () {
            ...
            function logoutLogonSystem () {
                var oDeferredAsyncLogoutEvent = new jQuery.Deferred(),
                    oPromiseAsyncLogoutEvent = oDeferredAsyncLogoutEvent.promise(),
                    arrLogoutEventsPromises = [];
            ...
            function federatedLogout () {
                ...
                that._suppressOData();
                mSystems = that._getRemoteSystems();
        ...
        this.getFLPUrl = function (bIncludeHash) {
            var sUrl = utils.getLocationHref(),
                iHashPosition = sUrl.indexOf(UrlParsing.getShellHash(sUrl));
        ...
        /**
         * Returns FLP's platfrom as specified in the configuration
         *
    ...
    sap.ushell.bootstrap = function (sPlatform, mAdapterPackagesByPlatform) {
        ...
        createAdapter("Container", system, null, true /* async */).then(function (adapter) {
            adapter.load().then(function () {
                ...
                // Pre-load given UI Services to allow sync access (via getService()) w/o triggering a sync request (CSP safeguarding of Neo Platform)
            ...
            function handlePlugins (aServices) {
                ...
                getPlugins.then(function (oCDMSitePlugins) {
                    ...
                    // S/4 my home
                    try {
                        // Additional try/catch as jQuery's catch doesn't always catch all the errors.
                        oPluginManager.loadPlugins("EarlyLoading").catch(function () {
                            Log.error("Error loading S/4's my home plugin");
                        });
                    } catch (oErr) {
                        Log.error("Error loading S/4's my home plugin");
                    }
            ...
            function prepareNWBC () {
                if (utils.hasFLPReady2NotificationCapability()) {
                    // Notify SAP Buisness Client (NWBC), that the Container and its Services, are ready to be used and
