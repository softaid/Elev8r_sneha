...
sap.ui.define([
    ...
    AdapterBase.prototype._resolveTileByVizId = function (oTile, oSite) {
        ...
        if (sExternalUrl || sHash) {
            ...
            if (!this._oUrlParsingPromise) {
                this._oUrlParsingPromise = sap.ushell.Container.getServiceAsync("URLParsing");
            }
            this._oUrlParsingPromise.then(function (oUrlParsing) {
                if (oTile.target) {
                    var oTarget = deepClone(oTile.target);
                    sHash = oUtilsCdm.toHashFromTarget(readUtils.harmonizeTarget(oTarget), oUrlParsing);
                }
                oDeferred.resolve({
                    tileResolutionResult: oApp,
                    tileIntent: sHash
    ...
    AdapterBase.prototype._createTileComponentData = function (oTile, bIsCatalogTile, oResolutionResult) {
        var sTitle = bIsCatalogTile ? this.getCatalogTileTitle(oTile) : this.getTileTitle(oTile),
            ...
            sNumberUnit = oTile.numberUnit || (oResolutionResult.tileResolutionResult && oResolutionResult.tileResolutionResult.numberUnit),
            oComponentData = {
                properties: {},
                startupParameters: {}
            };
