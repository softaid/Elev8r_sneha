    ...
    var oHomepageManager = BaseObject.extend("sap.ushell.components.HomepageManager", {
        ...
        _getPathOfTile: function (sTileId) {
            ...
            for (var nGroupIndex = 0; nGroupIndex < aGroups.length; ++nGroupIndex) {
                oGroup = aGroups[nGroupIndex];
                aTiles = oGroup.tiles;
                aLinks = oGroup.links;

                for (nTileIndex = 0; nTileIndex < aTiles.length; ++nTileIndex) {
                    if (aTiles[nTileIndex].uuid === sTileId) {
                        return "/groups/" + nGroupIndex + "/tiles/" + nTileIndex;
                    }
                }
                for (nLinkIndex = 0; nLinkIndex < aLinks.length; ++nLinkIndex) {
                ...
                that.oPageOperationAdapter.resetGroup(oGroupModel, bDefaultGroup).then(
                    function (oResetedObject) {
                        that._handleAfterSortable(function (sGroupId, oOldGroupModel, oResetedGroupModel) {
                            ...
                            oGroupControl = Core.byId("dashboardGroups").getGroupControlByGroupId(sGroupId);
        ...
        _deleteTile: function (sChannelId, sEventId, oData) {
            ...
            function focusNextTile (tileIndex, group, tileType) {
                var oDashboardGroups = Core.byId("dashboardGroups"),
                    oGroupControl = oDashboardGroups.getGroupControlByGroupId(group.groupId),
                    oNextTile;
            ...
            for (nGroupIndex = 0; nGroupIndex < aGroups.length; ++nGroupIndex) {
                oGroup = aGroups[nGroupIndex];
                for (nTileIndex = 0; nTileIndex < oGroup[sItems].length; ++nTileIndex) {
                    oTmpTile = oGroup[sItems][nTileIndex];
        ...
        _getTileInfo: function (aGroups, sTileId, sItems) {
            ...
            for (nTmpGroupIndex = 0; nTmpGroupIndex < aGroups.length; ++nTmpGroupIndex) {
                oTmpGroup = aGroups[nTmpGroupIndex];
                for (nTmpTileIndex = 0; nTmpTileIndex < oTmpGroup[sItems].length; ++nTmpTileIndex) {
                    oTmpTile = oTmpGroup[sItems][nTmpTileIndex];
                    if (oTmpTile.uuid === sTileId) {
        ...
        // TODO: reduce function complexity and remove the "eslint-disable-line complexity"
        /*
         * oData should have the following parameters:
         * fromGroupId
         * toGroupId
         ...
         */
        _moveTile: function (sChannelId, sEventId, oData) { // eslint-disable-line complexity
        ...
        createGroupsModelFrame: function (aGroups, personalization) {
            ...
            for (iGroupIndex = 0; iGroupIndex < aGroups.length; iGroupIndex++) {
                oOrgGroup = aGroups[iGroupIndex];
        ...
        _splitGroups: function (aGroups, iFirstVisibleGroupIndex) {
            var iGroupIndex,
                ...
                bEnablePersonalization = this.oModel.getProperty("/personalization"),
        ...
        getSegmentContentViews: function (groupSegment) {
            var nGroupSegmentIndex, nTilesIndex, oSegmentGroup, oSegmentTile;

            for (nGroupSegmentIndex = 0; nGroupSegmentIndex < groupSegment.length; nGroupSegmentIndex++) {
                oSegmentGroup = groupSegment[nGroupSegmentIndex];
        ...
        _initializeAnchorNavigationBar: function () {
            var oAnchorItemTemplate,
                oDashboardView = oHomepageManagerInstance.getDashboardView();
                ...
                if (!bTileBeingMoved && this.getScope && this.getScope() === "Actions") {
                    switch (oEvent.getParameters().action) {
                        case "Press":
                            ...
                            if (sState !== "Failed") {
                                sap.ui.require("sap/ushell/components/homepage/ActionMode")._openActionsMenu(oEvent);
                            }
                            break;
                        case "Remove":
        ...
        loadPersonalizedGroups: function () {
            ...
            this.oServiceLoadingPromise.then(function () {
