...
sap.ui.define([
    ...
    var oAPIs = {
        "sap.ushell.services.CrossApplicationNavigation": {
            oServiceCalls: {
                ...
                setInnerAppRoute: {
                    executeServiceCallFn: function (oServiceParams) {
                        var oHash = UrlParsing.parseShellHash(window.hasher.getHash()),
                            sNewHash;
                ...
                addOptionsActionSheetButton: {
                    executeServiceCallFn: function (oServiceParams) {
                        ...
                        aButtons.forEach(function (oButton) {
                            if (sap.ui.getCore().byId(oButton.id)) {
                                sap.ui.getCore().byId(oButton.id).destroy();
                            }
                            /*eslint-disable no-new*/
                            new sap.m.Button({
                                id: oButton.id,
    ...
    function showUIBlocker (bShow) {
        if (bShow === true) {
            if (sap.ui.getCore().byId("shell-header")) {
                sap.ui.getCore().byId("shell-header").setBlocked(true);
                var oHeaderElement = jQuery("#shell-header-blockedLayer");
                oHeaderElement.addClass("sapUshellShellBlocked");
            }
            if (sap.ui.getCore().byId("menuBar")) {
                // This is a specific case for blocking menuPlugin items
                sap.ui.getCore().byId("menuBar").setBlocked(true);
                var oMenuBarElement = jQuery("#menuBar-blockedLayer");
                oMenuBarElement.addClass("sapUshellMenuBarBlocked");
            }
        } else if (bShow === false) {
            if (sap.ui.getCore().byId("shell-header")) {
                sap.ui.getCore().byId("shell-header").setBlocked(false);
            }
            if (sap.ui.getCore().byId("menuBar")) {
                // This is a specific case for blocking menuPlugin items
                sap.ui.getCore().byId("menuBar").setBlocked(false);
            }
        }
    }
