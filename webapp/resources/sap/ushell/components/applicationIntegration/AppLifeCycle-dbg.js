...
sap.ui.define([
    ...
    function AppLifeCycle () {
        ...
        this.handleExitStateful = function (sFromId, oFrom, isHomePage) {
            ...
            if (Storage.get(sFromId)) {
                ...
                if (RelatedServices.isBackNavigation() === true && !isHomePage) {
                    sap.ui.getCore().getEventBus().publish("sap.ushell", "appClosed", oFrom);
        ...
        this.isAppOfTypeCached = function (appId, bIsAppOfTypeCachable, oKeepAliveMode) {
            ...
            sKeepAlive = new UriParameters(window.location.href).get("sap-keep-alive");
        ...
        this.isCachedEnabledAsAppParameter = function (oShellHash, oTarget, oKeepAliveMode) {
            ...
            if (oTarget && oTarget.url) {
                sKeepAlive = new UriParameters(oTarget.url).get("sap-keep-alive");
                if (sKeepAlive === oKeepAliveModes.FULL || sKeepAlive === oKeepAliveModes.RESTRICTED) {
                    if (oKeepAliveMode) {
                        oKeepAliveMode.paramValue = sKeepAlive;
                    }
        ...
        this.reuseStateFulContainerAndRestore = function (oApplicationContainer, sToId, oHandler, sFixedShellHash, oTarget) {
            var that = this,
            ...
            // invoke the life cycle interface "setInitialConfiguration" for the restored application
            return oHandler.restore(oApplicationContainer, sStorageId, oTarget)
                .then(function () {
        ...
        this.reuseStateFulContainer = function (oApplicationContainer, url, sToId, oHandler, sFixedShellHash, oTarget) {
            var that = this;
            ...
            // invoke the life cycle interface "setInitialConfiguration" for the restored application
            return oHandler.create(oApplicationContainer, url, sToId, oTarget)
                .then(function () {
                    that.showApplicationContainer(oApplicationContainer);
                }, function (vError) {
                    Log.error(vError && vError.message || vError);
        ...
        this.handleOpenStateful = function (bIsInitial, bIsInCache, sToId, oInnerControl, oTarget, sFixedShellHash, oKeepAliveMode) {
            ...
            if (Storage.get(sToId) && bIsInitial === false) {
                oPromise = this.reuseStateFulContainerAndRestore(oInnerControl, sToId, oHandler, sFixedShellHash, oTarget);
            } else {
                oPromise = this.reuseStateFulContainer(oInnerControl, oTarget.url, sToId, oHandler, sFixedShellHash, oTarget);
        ...
        this.handleControl = function (sIntent, sAppId, oShellHash, oTarget, fnWrapper, sFixedShellHash, sOldFixedShellHash) {
            ...
            if (typeof sOldFixedShellHash === "string") {
                ...
                if (oOldAppStorageEntry) {
                    ...
                    if (sCurrentPageId) {
                        oCurrentAppContainer = sap.ui.getCore().byId(sCurrentPageId);
                    }
                }
            }
        ...
        this.switchViewState = function (sState, bSaveLastState, sAppId) {
            ...
            if (sState === "searchResults") {
                ...
                if (!window.hasher.getHash().indexOf("Action-search") === 0) {
                    ...
                    window.hasher.setHash("Action-search&/searchTerm=" +
