    ...
    function LaunchPage (oAdapter/*, oContainerInterface*/) {
        ...
        this.addTile = function (oCatalogTile, oGroup) {
            return this._getAdapter(oGroup.contentProvider).addTile(oCatalogTile, oGroup)
                .fail(function () {
                    Log.error("Fail to add Tile: " + that.getCatalogTileId(oCatalogTile));
                });
        };
        ...
        this.getTileView = function (oTile) {
            ...
            /**
             * Remove tabindex from tileview, as the FLP is using a wrapper with its own tabindex.
             */
            oDfd.done(function (tileView) {
                if (tileView && tileView.addEventDelegate && tileView.getDomRef) {
                    tileView.addEventDelegate({
                        onBeforeRendering: function () {
                            var oDomRef = tileView.getDomRef();
                            if (oDomRef) {
                                var sOldValue = oDomRef.getAttribute("data-oldTabindex");
                                if (sOldValue) {
                                    oDomRef.setAttribute("tabindex", oDomRef.getAttribute("data-oldTabindex"));
