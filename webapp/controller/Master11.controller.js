		...
		return BaseController.extend("sap.ui.elev8rerp.componentcontainer.controller.Master", {
			...
			onUserNamePress: function (oEvent) {
				...
				var fnCompanySettingPress = function(oEvent){
					...
					gRoute.navTo("companySettings");
					navBtn.setVisible(false);
				};

				var currentContext = this;
				var oSideNavigation = currentContext.byId('oSideNavigation');
				var oNavigationList = currentContext.byId('oNavigationList');
				var oFixedNavigationList = currentContext.byId('oFixedNavigationList');
				var oSideNav = new JSONModel("model/CompanySetting.json");

				//Company setting menu
				var initCompanySetting = function(oEvent){

					
					
					oSideNav.attachRequestCompleted(function (oEvent) {
						var navigation = oSideNav.getData("/")["navigation"];
						// var fixednavigation = oModelSideNav.getData("/")["fixedNavigation"];
