...
], function (JSONModel, BaseController, subcontractorService) {

    "use strict";

    return BaseController.extend("sap.ui.elev8rerp.componentcontainer.controller.Masters.SubContractor", {

        onInit: function () {
            this.bus = sap.ui.getCore().getEventBus();
            this.bus.subscribe("master", "setDetailPage", this.setDetailPage, this);
            this.oFlexibleColumnLayout = this.byId("fclSubcontractor");

            this.bus = sap.ui.getCore().getEventBus();
            this.bus.subscribe("loaddata", "loadData", this.loadData, this);

            var emptyModel = this.getModelDefault();
            let detailModel= this.getView().getModel("")

            var subContractorModel = new JSONModel();
            subContractorModel.setData(emptyModel);
        ...
        onListItemPress: function (oEvent) {
            ...
            var viewModel = oEvent.getSource().getBindingContext("subContractorModel");
            var model = {
                "id": viewModel.getProperty("id"),
                "subconname": viewModel.getProperty("subconname"),
                "designation": viewModel.getProperty("designation"),
                "email": viewModel.getProperty("email"),
                "mobileno": viewModel.getProperty("mobileno"),
                "row1": "true",
            }
            this.bus = sap.ui.getCore().getEventBus();
            this.bus.publish("master", "setDetailPage", { viewName: "SubContractorDetail", viewModel: model });
        },
        ...
        setDetailPage: function (channel, event, data) {
            this.detailView = sap.ui.view({
                viewName: "sap.ui.elev8rerp.componentcontainer.view.Masters.SubContractorDetail",
                type: "XML"
            });
        ...
        loadData: function () {
            ...
            subcontractorService.getAllSubcontractors(function (data) {
                var oModel = new sap.ui.model.json.JSONModel();
                if (data.length && data[0].length) {
                    oModel.setData({ modelData: data[0] });
                    currentContext.getView().setModel(oModel, "subContractorModel");
                } else {
        ...
        onAddIconPress: function (oEvent) {
            ...
            var oModel = this.getView().getModel("subContractorModel");

            var model = {
                id: null,
                subconname: oModel.oData.subconname,
                designation: oModel.oData.designation,
                email: oModel.oData.email,
                ...
                onClose: function (sAction) {
                    if (sAction == "Ok") {
                        ...
                        commonService.importMaster(model, function (data) {
                            console.log(data)
                            var pModel = currentContext.getView().getModel("subContractorModel");
                            setTimeout(function () {
