sap.ui.define([
	...
	'sap/ui/elev8rerp/componentcontainer/services/Application/RoleAccess.service',
	'sap/ui/elev8rerp/componentcontainer/formatter/common.formatter',
					...
					commonFunction, commonService, manageEntityService, roleAccessService,
					commonFormatter) {
	"use strict";

	return BaseController.extend("sap.ui.elev8rerp.componentcontainer.controller.Application.RoleAccess", {
		...
		_checkAndSetPermission: function(obj, state){
			console.log('_checkAndSetPermission : ', obj);
			var arrPerm = "";
			if(state == "Unchecked"){
				arrPerm = "";
			}
		...
		getRoles: function (itemid) {
			...
			roleAccessService.getRoles(function (data) {
		...
		getPermissions: function (itemid) {
			...
			roleAccessService.getPermissions(function (data) {
				var oModel = new sap.ui.model.json.JSONModel();
				oModel.setData({ modelData: data[0] });
				...
				MessageBox.confirm(
					"Are you sure you want to delete?", {
						...
						onClose: function (sAction) {
							if (sAction == "OK") {
								roleAccessService.deletePerfomaceObjectives(currentContext.model, function (data) {
								});
								currentContext.bus = sap.ui.getCore().getEventBus();
								currentContext.bus.publish("onperformanceObjDelete", "onperformanceObjDelete", { data: currentContext.model });
		...
		onAgeChange: function (oEvent) {
			commonFunction.isRequired(this, "txtAge", "Please enter age in week.");
			commonFunction.isNumber(this, "txtAge")
		},

		onDecimalChange: function (oEvent) {
			var inputId = oEvent.getParameter('id');		
			commonFunction.isDecimal(this, inputId)
		},
		onPercentageChange: function (oEvent) {
			var inputId = oEvent.getParameter('id');			
			commonFunction.isPercentage(this, inputId)
		},

		validateForm: function () {
			var isValid = true;

			if (!commonFunction.isRequired(this, "txtAge", "Please enter age in week"))
				isValid = false;

			if (!commonFunction.isRequired(this, "txtLivability", "Please enter Liveability."))
				isValid = false;

			if (!commonFunction.isRequired(this, "txtHD", "Please enter HD."))
				isValid = false;
