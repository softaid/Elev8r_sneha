sap.ui.define([
	...
	return BaseController.extend("sap.ui.elev8rerp.componentcontainer.controller.Sales.BreederSales.InvoiceGenerations", {
		
		onInit: function () {
			this.bus = sap.ui.getCore().getEventBus();
            this.bus.subscribe("invoicegeneration", "setDetailPage", this.setDetailPage, this);     
            this.oFlexibleColumnLayout = this.byId("fclInvoiceGeneration");

			// set model for list dialog
			var oModel = new JSONModel(jQuery.sap.getModulePath("sap.ui.elev8rerp.componentcontainer.model.Sales.BreederSales", "/invoice.json"));
		...
		{
            this.bus = sap.ui.getCore().getEventBus();
            this.bus.publish("invoicegeneration", "setDetailPage", {viewName:"InvoiceGenerationContentDetail"});
		},
        ...
        onListIconPress: function(oEvent) {
		   if (!this._oDialog) {
			  this._oDialog = sap.ui.xmlfragment("sap.ui.elev8rerp.componentcontainer.fragmentview.Sales.BreederSales.InvoiceDialog", this);
		   }
				...
				cells : [ new sap.m.Text({
						...
						text : "{dataModel>location}"
							 ...
							 press: function (e) {
								 ...
								 currentContext.bus.publish("invoicegeneration", "setDetailPage", {viewName:"InvoiceGenerationContentDetail", viewModel : model});
							 }
					  })
				  ]
			});
