sap.ui.define([
	...
	return BaseController.extend("sap.ui.elev8rerp.componentcontainer.controller.Sales.CBFSales.SalesInvoice", {
		
		onInit: function () {
			this.bus = sap.ui.getCore().getEventBus();
            this.bus.subscribe("cbfsalesinvoice", "setDetailPage", this.setDetailPage, this);     
            this.oFlexibleColumnLayout = this.byId("fclCBFSalesInvoice");

			// set model for list dialog
			var oModel = new JSONModel(jQuery.sap.getModulePath("sap.ui.elev8rerp.componentcontainer.model.Sales.CBFSales", "/salesinvoice.json"));
		...
		{
            this.bus = sap.ui.getCore().getEventBus();
            this.bus.publish("cbfsalesinvoice", "setDetailPage", {viewName:"SalesInvoiceContentDetail"});
		},
        ...
        onListIconPress: function(oEvent) {
		   if (!this._oDialog) {
			  this._oDialog = sap.ui.xmlfragment("sap.ui.elev8rerp.componentcontainer.fragmentview.Sales.CBFSales.SalesInvoiceDialog", this);
		   }
	    ...
	    bindDetailTable : function(oModel)
	   {
		   ...
		   var oTable = this.getView().byId("tblCBFSalesInvoiceContent");
				...
				cells : [ new sap.m.Text({
						...
						text : "{dataModel>location}"
							 ...
							 press: function (e) {
								 ...
								 currentContext.bus.publish("cbfsalesinvoice", "setDetailPage", {viewName:"SalesInvoiceContentDetail", viewModel : model});
							 }
					  })
				  ]
			});
