sap.ui.define([
    ...
    return BaseController.extend("sap.ui.elev8rerp.componentcontainer.controller.Reports.CBF.Pending_Payment_GC_Report", {
        ...
        pendingGCReport: function () {
            ...
            if (this.validateForm()) {
                ...
                cBFReportsService.getPaymentPendingGCReport({ branchid: branchStr }, function async(data) {
        ...
        // generate PDF for Pending Payment GC Report
        onPdfExport: function () {
            var fullHtml = "";
            var headertable1 = "";
            headertable1 += "<!DOCTYPE html> <html> <head> <title>" + "Pending Payment GC Report" + "</title>" +
            ...
            // headertable1 += "widths: ['*','*','*','*','*'],";
            headertable1 += " body: [";
            headertable1 += "[ {text: 'Branch Name', style: 'tableHeader'}, {text: 'Farmer Name', style: 'tableHeader'},{text: 'Shed Name', style: 'tableHeader'},{text: 'Batch Name', style: 'tableHeader'},{text: 'Total Placement', style: 'tableHeader'},{text: 'Total Sold Bird', style: 'tableHeader'},{text: 'Total Sold weight', style: 'tableHeader'},{text: 'Average Weight', style: 'tableHeader'},{text: 'Average Selling Rate', style: 'tableHeader'},{text: 'Production Cost/Kg', style: 'tableHeader'},{text: 'FCR', style: 'tableHeader'},{text: 'CFCR', style: 'tableHeader'},{text: 'Total Growing Amount', style: 'tableHeader'},{text: 'TDS %', style: 'tableHeader'},{text: 'TDS Amount', style: 'tableHeader'},{text: 'Net Growing Changes Payable', style: 'tableHeader'}],";

            for (var i = 0; i < tbleModel.length; i++) {
                if (tbleModel[i].branchname == null) {
                    tbleModel[i].branchname = "-";
                }
                if (tbleModel[i].farmer_name == null) {
                    tbleModel[i].farmer_name = "-";
                }
                if (tbleModel[i].shed_name == null) {
                    tbleModel[i].shed_name = "-";
                }
                if (tbleModel[i].batch_number == null) {
                    tbleModel[i].batch_number = "-";
                }
                if (tbleModel[i].batchplaceqty == null) {
                    tbleModel[i].batchplaceqty = "-";
                }
                if (tbleModel[i].totalsaleqty == null) {
                    tbleModel[i].totalsaleqty = "-";
                }
                if (tbleModel[i].totalsaleweight == null) {
                    tbleModel[i].totalsaleweight = "-";
                }
                if (tbleModel[i].avgweightofbird == null) {
                    tbleModel[i].avgweightofbird = "-";
                }

                if (tbleModel[i].avgsellingrate == null) {
                    tbleModel[i].avgsellingrate = "-";
                }
                if (tbleModel[i].productioncostperkg == null) {
                    tbleModel[i].productioncostperkg = "-";
                }
                if (tbleModel[i].fcr == null) {
                    tbleModel[i].fcr = "-";
                }
                if (tbleModel[i].cfcr == null) {
                    tbleModel[i].cfcr = "-";
                }
                if (tbleModel[i].rearingchargepayable == null) {
                    tbleModel[i].rearingchargepayable = "-";
                }
                if (tbleModel[i].tdsper == null) {
                    tbleModel[i].tdsper = "-";
                }
                if (tbleModel[i].tds == null) {
                    tbleModel[i].tds = "-";
                }
                if (tbleModel[i].netpayableamt == null) {
                    tbleModel[i].netpayableamt = "-";
                }

                headertable1 += "['" + tbleModel[i].branchname + "','" + tbleModel[i].farmer_name + "','" + tbleModel[i].shed_name + "','" + tbleModel[i].batch_number + "','" + tbleModel[i].batchplaceqty + "','" + tbleModel[i].totalsaleqty + "','" + tbleModel[i].totalsaleweight + "','" + tbleModel[i].avgweightofbird + "','" + tbleModel[i].avgsellingrate + "','" + tbleModel[i].productioncostperkg + "','" + tbleModel[i].fcr + "','" + tbleModel[i].cfcr + "','" + tbleModel[i].rearingchargepayable + "','" + tbleModel[i].tdsper + "','" + tbleModel[i].tds + "','" + tbleModel[i].netpayableamt + "'],"
            }
            headertable1 += "]";
            headertable1 += "}";
            headertable1 += "}";
            headertable1 += "]," +
                ...
                "}" +
                "};" +
                "pdfMake.createPdf(docDefinition).download('Pending_Payment_GC_Report.pdf');" +
                "} });";
        ...
        onDataExport: sap.m.Table.prototype.exportData || function (oEvent) {
            ...
            var oExport = new Export({
                ...
                columns: [
                    ...
                    {
                        name: "Total Growing Amount",
                        template: { content: "{rearingchargepayable}" }
                    },

                    {
                        name: "TDS %",
                        template: { content: "{tdsper}" }
                    },
                    {
                        name: "TDS Amount",
                    ...
                    {
                        name: "Net Growing Changes Payable",
