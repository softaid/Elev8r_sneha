sap.ui.define([
    ...
    return BaseController.extend("sap.ui.elev8rerp.componentcontainer.controller.Reports.Breeder.ParentBirdBalance", {
        ...
        onSearchFlockExpencesBeforeReport: function () {
            if (this.validateForm()) {
                ...
                breederReportsService.ParentBirdBalanceReport({ batchid: this.batchesStr, fromdate: fromdate, todate: todate }, function async(data) {
        ...
        //     if (!commonFunction.ismultiComRequired(this, "txtBreederToBatch", "Batch is required is required"))
        //         isValid = false;
        //     return isValid;
        // },
        ...
        replaceTemplateData: function (template) {
            ...
            for (var indx in tbleModel) {
                ...
                htmTable += "<tr>";
                htmTable += "<td align='left'>" + model["batchname"] + "</td>"
                htmTable += "<td align='left'>" + model["phaseststus"] + "</td>"
                htmTable += "<td align='right'>" + model["ageinweek"] + "</td>"
                htmTable += "<td align='right'>" + model["liveqty"] + "</td>"
                htmTable += "<td>" + model["productivebird"] + "</td>"
                htmTable += "<td align='right'>" + model["mortality"] + "</td>"
                htmTable += "<td align='right'>" + model["purvalue"] + "</td>"
                htmTable += "<td align='right'>" + model["feedcost"] + "</td>"
                htmTable += "<td align='right'>" + model["medicost"] + "</td>"
                htmTable += "<td align='right'>" + model["vaccost"] + "</td>"
                htmTable += "<td align='right'>" + model["vitmcost"] + "</td>"
                htmTable += "<td align='right'>" + model["transfercost"] + "</td>"
                htmTable += "<td align='right'>" + model["salesvalue"] + "</td>"
                htmTable += "<td align='right'>" + model["mortality"] + "</td>"
                htmTable += "<td align='right'>" + model["ohexp"] + "</td>"
                htmTable += "<td align='right'>" + model["otexp"] + "</td>"
                htmTable += "<td align='right'>" + model["amortizationvalue"] + "</td>"
                htmTable += "<td align='right'>" + model["balancevalue"] + "</td>"
                htmTable += "<td align='right'>" + model["birdcost"] + "</td>"
                htmTable += "</tr>";
            }
        ...
        createPDF: function () {
            var currentContext = this;
            commonFunction.getHtmlTemplate("Breeder", "ParentBirdBalanceReport.template.html", function (dataHtml) {
        ...
        onDataExport: sap.m.Table.prototype.exportData || function (oEvent) {
            ...
            var oExport = new Export({
                ...
                columns: [
                    ...
                    {
                        name: "Pro Bird",
                        template: { content: "{productivebird}" }
                    },
                    {
                        name: "Mortality",
                    ...
                    {
                        name: "Medicine Cost",
                        template: { content: "{medicost}" }
                    },
                    {
                        name: "Vaccine Cost",
                        template: { content: "{vaccost}" }
                    },
                    {
                        name: "Vitamin",
                        template: { content: "{vitmcost}" }
                    },
                    ...
                    {
                        name: "OH Cost",
                        template: { content: "{ohexp}" }
                    },
                    {
                        name: "Other Expences",
                        template: { content: "{otexp}" }
                    },
                    {
                        name: "Amortization",
