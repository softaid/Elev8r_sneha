sap.ui.define([
    ...
    return BaseController.extend("sap.ui.elev8rerp.componentcontainer.controller.Reports.Breeder.BatchwiseProfitAndLossReport", {
        ...
        onInit: function () {
            ...
            this.getView().setModel(model, "batchpandlModel");

            var currentContext = this;
            commonService.getAllBreederBatchesByStatusid({statusid : 222},function(data){
        ...
        onBreederBatchDialogClose: function (oEvent) {
            ...
            if (aContexts != undefined) {
                ...
                var oModel = currentContext.getView().getModel("batchpandlModel");
        ...
        resetModel: function () {
            var tbleModel = this.getView().getModel("batchwiseIncomeModel");
            tbleModel.setData({ modelData: [] });

            var tbleModel1 = this.getView().getModel("batchwiseExpensesModel");
            tbleModel1.setData({ modelData: [] });
        ...
        onSearchData: function () {
            if (this.validateForm()) {
                var currentContext = this;
                var oModel = this.getView().getModel("batchpandlModel").oData
                ...
                breederReportsService.getbatchwiseprofitandlossReport(oModel, function (data) {
                    console.log(data);

                    data[0].push({"particulars" : "", "amount" : "Total Expenses : "+data[2][0].totalexpenses},
                    {"particulars" : "Net Profit/Loss : "+data[2][0].netporl, "amount" : ""});
                    var eModel = new sap.ui.model.json.JSONModel();
                    eModel.setData({ modelData: data[0]});
                    currentContext.getView().setModel(eModel, "batchwiseExpensesModel");
                    
                    data[1].push({"particulars" : "", "amount" : "Total Income : "+data[2][0].totalincome});
                    var iModel = new sap.ui.model.json.JSONModel();
                    iModel.setData({ modelData: data[1] });
                    currentContext.getView().setModel(iModel, "batchwiseIncomeModel");
