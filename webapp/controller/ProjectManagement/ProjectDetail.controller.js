		...
		"sap.ui.elev8rerp.componentcontainer.controller.ProjectManagement.ProjectDetail",
		{
		  ...
		  onInit: function () {
			...
			this.bus.subscribe(
			  "projectdetailassign",
			  "handleAssignList",
			  this.handleAssignList,
			  this
			);
			this.bus.subscribe(
			  "projectdetailapprove",
			  "handleApproveList",
			  this.handleApproveList,
			  this
			);
		  ...
		  onListItemPressStage: function (oEvent, obj) {
			...
			let oDayHistory =
			  oEvent?.getSource()?.getBindingContext("tblModel")?.getObject() ??
			  obj;
			let projectModel = this.getView().getModel("projectModel").getData();
			...
			oDayHistory.isparent = oDayHistory.isparent === 1 || oDayHistory.isparent === true? true: false;
			oDayHistory.iscompleted =
			  oDayHistory.iscompleted === 1 || oDayHistory.iscompleted === true
				? true
				: false;
		  ...
		  onListItemPressActivity: function (oEvent, obj) {
			...
			oDayHistory.isactive =
			  oDayHistory.isactive == 1 || oDayHistory.isactive == true? true: false;
			  oDayHistory.isparent == 1 || oDayHistory.isparent == true? true: false;
			oDayHistory.iscompleted =
			  oDayHistory.iscompleted === 1 || oDayHistory.iscompleted === true
				? true
				: false;
			...
			Projectservice.getStageOrActivityDetail(
			  ...
			  function (data) {
				...
				// if dependencyStatus  is true means all dependency stage are completed  so we can proceed it  otherwise false mean  condition is not satisfied
				oDayHistory.dependencyStatus = dependency !== null && dependency !== "" ? dependency.split(",").every((ele) => {
						return currentContext.projectCompletionObj[ele] == 100;
					  })
					: true;
			...
			if (field == "Activity") {
			  ...
			  Projectservice.getStageOrActivityDetail(
				...
				function (data) {
				  ...
				  detail.isparent = detail.isparent === 1 ? true : false;
				  detail.isstarted =
