    ...
    return BaseController.extend(
      "sap.ui.elev8rerp.componentcontainer.controller.ProjectManagement.ProjectStageDetail",
      {
        ...
        onInit: function () {
          ...
          this.getView().byId("stageid").setVisible(false);
        ...
        isParentChange: function (oEvent) {
          ...
          if (isparentState === true) {
            this.getView().byId("stageid").setVisible(true);
            this.getView().byId("ParentStagePercentage").setVisible(true);
            //commonFunction.getReferenceByTypeCodeAndParentTypeStage("ProMilestones", "Stage", "parentStageModel", this);
          } else {
            this.getView().byId("stageid").setVisible(false);
            this.getView().byId("ParentStagePercentage").setVisible(false);
          }
        },

        onBeforeRendering: async function () {
          let StageDetailModel =
            this.getView().getModel("StageDetailModel").oData;
          this.getView().getModel("stageModel").oData;
          console.log(
            "---------Stage Model in stage----------",
            this.getView().getModel("stageModel").oData
          );
          var currentContext = this;
          ...
          if (StageDetailModel.id != undefined) {
            ...
            if (StageDetailModel.parentid !== null) {
              this.getView().byId("projectPercentele").setVisible(false);
            } else {
              this.getView().byId("ParentStagePercentage").setVisible(false);
            }
          ...
          currentContext.getUserByDepartment(
            StageDetailModel?.departmentid ?? "notSelect"
          );
        },
        ...
        getUserByDepartment: function (departmentid) {
          ...
          if (departmentid != "notSelect") {
            ManageUserService.getUserByDepartment(
            ...
            MessageToast.show(
              "Please Select Department to set assign to and approve by"
            );
          }
        },
        ...
        onDepartmentChange: function () {
          let currentContext = this;
          let StageDetailModel = currentContext
            .getView()
            .getModel("StageDetailModel").oData;
        ...
        onSave: function () {
         // if (this.validateForm()) {
            ...
            var assigntostring = this.getView()
              .byId("assigntoid")
              .getSelectedItem();
            var assignStr = assigntostring.mProperties.key;
            var approvebystring = this.getView()
              .byId("assignbyid")
              .getSelectedItem();
            ...
            if (RoleData.length) {
              for (var p = 0; p < RoleData.length; p++) {
                if (
                  assignStr == RoleData[p].id &&
                  RoleData[p].userrole != "Administrator"
                ) {
                  assignMailId = RoleData[p].email;
                } else if (
                  approvebyStr == RoleData[p].id &&
                  RoleData[p].userrole != "Administrator"
                ) {
            ...
            oModel.isparent = oModel.isparent === true ? 1 : 0;
            (oModel.stagecompletionpercentage =
              oModel.parentid !== null
                ? parseInt(oModel.parentstagecompletionpercentage)
                : oModel.stagecompletionpercentage),
            ...
            console.log("----------StageoModel  save data----------", oModel);

            if (oModel.parentid !== null && oModel.iscompleted == true) {
              parseInt(oModel.parentstagecompletionpercentage); //Input Value
              let currentstageper = 100 - oModel.parentstageper;
              console.log("-----currentstageper---", currentstageper);
              let completionPer = oModel.parentstageper + parseInt(oModel.parentstagecompletionpercentage);
              console.log("-----completionPer---", completionPer);
              if (currentstageper >=parseInt(oModel.parentstagecompletionpercentage)) {
                let totalPer = parseInt(oModel.parentstagecompletionpercentage) + oModel.parentstageper;
                console.log("------------totalPer-----------", totalPer);
                //oModel.stagecompletionpercentage = totalPer;
				oModel.totalPer = totalPer;

                Projectservice.updateProjectManagementStageComPer(oModel,function (savedataupdateddata) {
                    console.log("-----savedataupdateddata-----",savedataupdateddata);
            ...
            Projectservice.saveProjectActivityDetail(
              oModel,
              function (savedata) {
                    ...
                    if (document.id == undefined) {
                      Projectservice.saveDocumentCollectionDetails(
                        ...
                        function (obj) {
                          ...
                          var message =
                            parentModel.id == null ? saveMsg : editMsg;
                          if (message == null) {
            ...
            currentContext.DeleteDocumentArr.length > 0
              ? currentContext.onDeleteDocumentSave()
              : "No image is available to delete";
            if (oModel.dependencyStatus == false) {
              MessageToast.show(
                "Please need to first complete the prerequisite  stage for  starting the current stage"
              );
            }
        ...
        handleStageApproveToggle: function () {
          ...
          if (
            oModel.assignedby != approveById ||
            sessionData.roleNames != "AD"
          ) {
            ...
            oModel.iscompleted = false;
            MessageToast.show(
              "stage is not completed yet so first complete the stage then approve it"
            );
            currentContext.getView().getModel("StageDetailModel").refresh();
          }
        },
        ...
        handleProjectCompPer: function () {
          ...
          let oModel = currentContext
            ...
            .getModel("StageDetailModel").oData;
          console.log(
            "----------StageDetailModel for Parent stage cal------------",
            oModel
          );
          ...
          if (oModel.iscompleted == true) {
            const obj = {
              ...
              startdate:
                oModel.projectDetail.startdate != null
                  ? commonFunction.getDate(oModel.projectDetail.startdate)
                  : oModel.projectDetail.startdate,
              enddate:
                oModel.projectDetail.enddate != null
                  ? commonFunction.getDate(oModel.projectDetail.enddate)
                  : oModel.projectDetail.enddate,
              isactive:
                oModel.projectDetail.isactive === true ||
                oModel.stageDetail.isactive == 1
                  ? 1
                  : 0,
              isstd: oModel.projectDetail.isstd === true ? 1 : 0,
              isparent: oModel.projectDetail.isparent === true ? 1 : 0,
              userid: commonService.session("userId"),
              completionper:
                (oModel?.projectDetail?.completionper ?? 0) +
                (oModel?.projectweightage ?? 0),
              fromreference: 0,
            ...
            (obj.actualenddate =
              obj.completionper == 100
                ? commonFunction.getDate(oModel.actualenddate)
                : oModel.projectDetail.actualenddate != null
                ? commonFunction.getDate(oModel.projectDetail.actualenddate)
                : oModel?.projectDetail?.actualenddate ?? null),
              console.log("------------obj------------", obj);
            ...
            if (oModel.parentid !== null) {
              ...
              console.log("-----currentstageper---", currentstageper);
              let completionPer =
                oModel.parentstageper +
                parseInt(oModel.parentstagecompletionpercentage);
              console.log("-----completionPer---", completionPer);
              if (completionPer > 100) {
                MessageToast.show(
                  `Parent stage Percentage is ${completionPer}  and greater than 100 so please change it then save it`
                );
              }
            }
          }
        ...
        validateForm: async function () {
            ...
            if (oModel.startdate) {
              if (oModel.assignedby && oModel.assignedto) {
                ...
                return MessageToast.show(
                  `Before start the stage you need to select assign to and approve by`
                );
              }
            } else {
            ...
            if (oModel.actualstartdate) {
              if (oModel.assignedby && oModel.assignedto) {
                if (oModel.startdate && oModel.enddate) {
                  ...
                  return MessageToast.show(
                    `Before start the stage you need to select start date and end date `
                  );
                }
              } else {
                return MessageToast.show(
          ...
          Projectservice.getSumOfAllStageOrActivity(obj, async function (data) {
            if (oModel.parentid !== null) {
              ...
              if (completionPer > 100) {
                MessageToast.show(
                  `Sum of project weightage of all stage is ${completionPer}  and greater than 100 so please change it then save it`
                );
                isValid = false;
              } else {
                isValid = true;
